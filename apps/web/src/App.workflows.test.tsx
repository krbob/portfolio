...
describe('App', () => {
  ...
  it('renders targets tab via /strategy?tab=targets', async () => {
    globalThis.fetch = vi.fn(async (input) => {
      ...
      if (url.includes('/api/v1/auth/session')) {
        return new Response(
          JSON.stringify({ authEnabled: false, authenticated: true, mode: 'DISABLED' }),
          { status: 200 },
        )
      }
      ...
      if (url.includes('/api/v1/readiness')) {
        return new Response(
          JSON.stringify({ status: 'READY', checkedAt: '2026-03-13T12:00:00Z', checks: [] }),
          { status: 200 },
        )
      }
      ...
      if (url.includes('/api/v1/portfolio/rebalancing-settings')) {
        return new Response(
          JSON.stringify({ toleranceBandPctPoints: '5.00', mode: 'CONTRIBUTIONS_ONLY' }),
          { status: 200 },
        )
      }
      ...
      if (url.includes('/api/v1/portfolio/allocation')) {
        return new Response(
          JSON.stringify({
            asOf: '2026-03-13', valuationState: 'MARK_TO_MARKET', configured: false,
            toleranceBandPctPoints: '5.00', rebalancingMode: 'CONTRIBUTIONS_ONLY',
            targetWeightSumPct: '0.00', totalCurrentValuePln: '2000.00', availableCashPln: '0.00',
      ...
      if (url.includes('/api/v1/portfolio/benchmark-settings')) {
        return new Response(JSON.stringify({ enabledKeys: [], pinnedKeys: [], customBenchmarks: [] }), { status: 200 })
      }

      if (url.includes('/api/v1/portfolio/data-quality')) {
        return new Response(JSON.stringify({ status: 'PASS', checks: [], warningCount: 0 }), { status: 200 })
      }
      ...
      if (url.includes('/api/v1/portfolio/read-model-refresh')) {
        return new Response(JSON.stringify({
          ...
          lastTrigger: null, lastDurationMs: null, modelNames: [],
        }), { status: 200 })
      }
      ...
      if (url.includes('/api/v1/market-data/snapshots')) {
        return new Response(JSON.stringify([]), { status: 200 })
      }
  ...
  it('does not leak raw UNCONFIGURED status in the contribution planner', async () => {
    globalThis.fetch = vi.fn(async (input) => {
      const url = typeof input === 'string' ? input : input instanceof Request ? input.url : String(input)
      ...
      if (url.includes('/api/v1/portfolio/rebalancing-settings')) {
        return new Response(
          JSON.stringify({ toleranceBandPctPoints: '2.00', mode: 'CONTRIBUTIONS_ONLY' }),
          { status: 200 },
        )
      }

      if (url.includes('/api/v1/portfolio/allocation/contribution-plan')) {
        return new Response(
          JSON.stringify({
            amountPln: '120000.00',
            projected: {
      ...
      if (url.includes('/api/v1/portfolio/allocation')) {
        return new Response(
          JSON.stringify({
            ...
            rebalancingMode: 'CONTRIBUTIONS_ONLY',
            targetWeightSumPct: '100.00',
            totalCurrentValuePln: '327135.00',
            availableCashPln: '4.48',
            breachedBucketCount: 0,
            largestBandBreachPctPoints: null,
            recommendedAction: 'WITHIN_TOLERANCE',
    ...
    fireEvent.change(await screen.findByLabelText(/contribution amount|kwota wpłaty/i), {
      target: { value: '120000' },
    })
    fireEvent.click(screen.getByRole('button', { name: /calculate contribution plan|przelicz plan wpłaty/i }))

    expect((await screen.findAllByText(/post-contribution value|wartość po wpłacie/i)).length).toBeGreaterThan(0)
    expect(screen.queryByText('UNCONFIGURED')).not.toBeInTheDocument()
    expect(screen.getAllByText(/not configured|brak konfiguracji/i).length).toBeGreaterThan(0)
  })
